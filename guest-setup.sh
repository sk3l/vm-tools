...
USAGE="usage : guest-setup.sh <name> --img <path> --disk <path>\
...
$(virsh vol-info --pool images $GUEST_NAME.qcow2 > /dev/null 2>&1)
if [[ $?  -ne 0 ]]; then
   echo "Creating volume $GUEST_NAME.qcow2"
   virsh vol-create-as images $GUEST_NAME.qcow2 "$VOL_SIZE"G --format qcow2
fi
